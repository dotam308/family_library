...
<nav class="navigation pagination text-center">
    ...
    <div class="nav-links">
        @if($c > 1)
            <a class="page-numbers" href="{{ $data->url(1).$sort .$search }}"><i class="fa fa-angle-double-left"></i></a>
            <a class="page-numbers" href="{{ $data->url($c-1).$sort .$search }}"><i class="fa fa-arrow-left"></i></a>
        @endif
        @for($i = $l; $i <= $r; $i++)
            @if($i != $c)
                <a class="page-numbers" href="{{ $data->url($i).$sort .$search }}">{{ $i }}</a>
            @else
                <span class="page-numbers current">{{ $i }}</span>
            @endif
        @endfor
        @if($c < $r)
            <a class="page-numbers" href="{{ $data->url($c+1).$sort .$search }}"><i class="fa fa-arrow-right"></i></a>
            <a class="page-numbers" href="{{ $data->url($lastPage).$sort .$search }}"><i class="fa fa-angle-double-right"></i></a>
        @endif
    </div>
</nav>